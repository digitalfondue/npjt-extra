...
public enum QueryType {
	...
	private static <T> AffectedRowCountAndKey<T> executeUpdateAndKeepKeys(
		...
		Object key;
		if (keys.size() > 1) {
			AutoGeneratedKey spec = Objects.requireNonNull(method.getDeclaredAnnotation(AutoGeneratedKey.class), "more than one key for query " + template + ": annotation @AutoGeneratedKey required");
